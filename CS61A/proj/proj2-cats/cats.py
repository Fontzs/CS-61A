...
def about(topic):
    ...
    def about_string(paragraph):
        s = split(paragraph)
        for i in range(0, len(s)):
            s[i] = lower(remove_punctuation(s[i]))
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for i in range(0, len(word_list)):
        if diff_function(typed_word, word_list[i], limit) < min_difference:
            index = i
            min_difference = diff_function(typed_word, word_list[i], limit)
...
def feline_fixes(typed, reference, limit):
    ...
    if typed == '' or reference == '':
        return len(typed) + len(reference)
...
def hidden_kittens(typed, reference, limit):
    ...
    def helper(a, b):
        if b == '':
            ...
            return helper(a[1:], b[1:]) + helper(a[1:], b)

    res = helper(typed, reference)
    if res == 0:
        return limit + 1
    return res
...
def report_progress(typed, prompt, user_id, upload):
    ...
    long_dick = {'id': user_id, 'progress': ratio}
    upload(long_dick)
    return ratio
    # END PROBLEM 8
...
def time_per_word(words, times_per_player):
    ...
    for i in range(0, len(times_per_player)):
        ...
        for j in range(1, len(times_per_player[i])):
            p[i].append(times_per_player[i][j] - times_per_player[i][j - 1])

    return match(words, p)
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    for i in word_indices:
        ...
        min_index = 0
        min_times = match['times'][0][i]

        for j in player_indices:
            if match['times'][j][i] < min_times:
                min_times = match['times'][j][i]
                min_index = j
        res[min_index].append(word)
